...
**問題核心**：每次 setup 都係手動 + 靠記性。新機越多，越證明你應該將個 setup 自動化。
...
唔好再喺 `.zshrc` 寫 `export ANTHROPIC_API_KEY="sk-ant-..."`。Plain text key 喺 dotfiles repo 即係 leak。
...
新機 setup：第一次手動 `security add-generic-password` 入 3-5 個 key，之後 `.zshrc` 自動 load。Dotfiles repo 唔再 hold 任何 secret。
...
Claude 出 script，你 review。確認無問題，commit 入 dotfiles repo 做 `bootstrap.sh`。
...
- 公司 `.claude/settings.json`（hook、permission default、共用 slash command）
...
你公司租咗部 Linux GPU 機 train model，要 setup Claude Code。
...
呢個 onboarding 嘅深層 insight：**每次手動 setup 都係未來自己嘅債**。

第一次手動 setup：1 個鐘
第二次手動 setup：1 個鐘（你已經唔記得上次點做）
第三次手動 setup：1 個鐘 + 開始悶
第四次：你終於受唔住，花 1 個下晝寫自動化

對比：第一次手動 setup 就用 Claude Code 一齊寫埋 bootstrap.sh。第一次成本：1.5 個鐘。第 2/3/4 次每次：30 分鐘。第 5 次（教新同事）：你 forward 一條 curl link，全程零分鐘。

呢個 mindset 嘅 reframe：**dotfiles + bootstrap script 唔係「dev 潔癖」，係 onboarding 嘅 ROI 投資**。每多寫一 line 自動化，未來嘅你（同你 team）少痛一次。

最後提醒：

- ✅ **secret 永遠唔好入 dotfiles repo**。Keychain / 1Password / pass —— 揀一樣。
- ⚠️ **bootstrap.sh 每 3 個月跑一次測試**（喺 throwaway VM 或者 Docker）。Brew formula 改名、API endpoint 搬位都會 break 你 script。
- 🎯 **dotfiles + Claude Code 嘅組合最大 leverage**：CLAUDE.md template + slash command + hooks 全部 sync。新機 30 分鐘等於舊機。
