...
呢個 onboarding 嘅深層 insight：**每次手動設置都係未來自己嘅債**。

第一次手動設置：1 個鐘第二次手動設置：1 個鐘（你已經唔記得上次點做）
第三次手動設置：1 個鐘 + 開始悶第四次：你終於受唔住，花 1 個下晝寫自動化

對比：第一次手動設置就用 Claude Code 一齊寫埋 bootstrap.sh。第一次成本：1.5 個鐘。第 2/3/4 次每次：30 分鐘。第 5 次（教新同事）：你 forward 一條 curl link，全程零分鐘。

呢個心態嘅 reframe：**dotfiles + bootstrap script 唔係「dev 潔癖」，係 onboarding 嘅回報投資**。每多寫一 line 自動化，未來嘅你（同你 team）少痛一次。
