...
呢個順序嘅根源：**任務之間唔互相 dependent** —— code review 唔等 test 覆蓋結果，test 覆蓋唔等保安掃描。但你 default 順序跑，因為個腦每次只可以集中 1 個任務。
...
之前你個腦 1 條 thread 順序行。即使多 monitor 都唔可以真正並行注意力。Subagent = 將並行外判 —— 5 個 subagent 同時跑 5 個任務，你主腦只負責編排 + 整合。
...
- 🎯 **自訂 subagent 1 個月會自然進化**。你每日 ship 嘅時候會發覺「I keep doing same review pattern」，codify 入可重用嘅 subagent 定義。
