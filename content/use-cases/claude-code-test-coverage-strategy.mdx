...
**2. Coverage 數字靚，但 test 喺 mock 一個 mock**
情境段講過嗰個 CSV export「3 個 test 全部 mock 一個 mock」。critical path 一樣會中招：你逼到 line + branch 99%，但每個 dependency 都 mock 做「永遠成功」，等於 test 緊個 mock，唔係 test 緊你嘅 logic。
...
**4. Claude 對「邊界情況」嘅想像力，受你個 prompt 框死**
critical path 階段一叫 Claude 列 input boundary、business rule edge、external failure。佢列得幾好，取決於佢估唔估到你個 domain 嘅暗坑。你冇講「呢個系統有 multi-currency」，佢就唔會諗到 currency rounding——正正就係情境段爆嘅嗰個。
