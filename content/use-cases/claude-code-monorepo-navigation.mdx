...
你開 Claude Code 喺 repo root，叫佢「fix 個 admin login 嘅 bug」。Claude 做啲乜？

1. Grep 成個 repo 搵 `login`
2. Match 到 23 個檔案，喺 `apps/web`、`apps/admin`、`packages/auth` 都有
3. Context 用咗 40k token 先理解到「邊個 login 先係 admin login」
4. 一唔小心改埋 `apps/web` 嘅 login flow，你 review 嗰陣先發現

Monorepo 嘅默契係「package 之間有界線」，但 Claude 預設行為係 flat search — 界線對佢嚟講唔存在。

呢篇拆 4 招：root CLAUDE.md 做地圖、per-package CLAUDE.md 做 scope、`/work-in` slash command 限制範圍、subagent 平行做唔同 package。

## 跟住做

### 1. Root CLAUDE.md：寫個 monorepo map

Repo root 嘅 `CLAUDE.md` 唔係 README — 係**畀 Claude 睇嘅導航文件**。重點：每個 package 嘅 job、boundary、邊個唔可以亂改。
...
呢份文件 Claude 入 session 時自動讀。佢有咗地圖，就唔會盲 grep。

### 2. Per-package CLAUDE.md：scope 內部規則

每個 package 自己嘅資料夾入面再放一份 `CLAUDE.md`，講該 package 嘅內部規矩。Claude Code 識自動 walk up 同 walk down 個 tree 揾呢啲 file。
...
Claude 入呢個 package 工作嗰陣，root + scoped 兩份 CLAUDE.md 都 load。Context 集中，亂跨 package 嘅機會大跌。

### 3. `/work-in` slash command：硬性 scope
...
用法：`/work-in packages/auth`。Claude 即刻收窄 scope，唔會去摸 `apps/web`。
...
每個 subagent 自己 context、自己 CLAUDE.md scope。Parent agent 只負責協調，唔處理檔案。Token 用量比 monolithic session 平 60-70%。
...
Claude 知道 build order 之後，做改動會主動建議「呢個會 cascade 去 X、Y、Z，建議跑 `turbo build --filter=...`」。

### 變化 2：pnpm workspace（無 Turborepo）

冇 task graph 嘅話，靠 `pnpm --filter` flag。Root CLAUDE.md 加：
...
Claude 學識 filter 之後，唔會再喺 root 撞 install。

### 變化 3：Mixed framework monorepo（Next.js + Astro + Vite）

3 個 app、3 個 framework，最易撞 import / config 衝突。Root CLAUDE.md 加 framework matrix：
...
Monorepo 嘅核心唔係「將代碼放埋一齊」— 係**邊界**。Package boundary 喺人類眼中係 folder structure，喺 Claude 眼中**預設係透明嘅**，因為佢嘅 default tool 係 flat grep。

你嘅工作係將呢啲邊界變返 visible：

- **Root CLAUDE.md** = 畀 Claude 個 monorepo map
- **Per-package CLAUDE.md** = 畀 Claude 該 package 嘅 local rules
- **Slash command** = 硬性收窄 scope，避免漂移
...
HK agency / SaaS shop 一單客有 3-5 個 product 同一個 monorepo 係常態。前期投資呢 4 招大概半日。回報係之後每個 task 慳 30-50% token + 唔再 review 到 cross-package mistake。值得做。
