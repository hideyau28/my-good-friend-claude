...
呢類風險共通 pattern：**唔係冇能力 review，係冇時間 + 冇紀律每次覆檢**。手動逐個 PR 逐個 file 對 OWASP top 10 + 掃 secret + 試 RBAC bypass —— 1 個 PR 起碼 1 個鐘，你冇可能每次做。
...
效果：每個 PR 自動觸發，review 結果 sticky comment 喺 PR 入面，CRITICAL finding block merge。
...
- 呢套設置完，跑 1 個月，calibrate false positive rate。Too noisy = tune `/security-review` prompt 收窄；漏嘢 = 加 check item。
